...
MoneyFlow 是一款使用 SwiftUI 与 SwiftData 构建的本地优先 iOS 专业财务规划工具。结合 **CFP（注册财务策划师）目标导向财富规划（GBWM）** 与 **CFA（特许金融分析师）资产负债压力测试** 体系，帮助用户看清现金流韧性、推演还贷加速策略、实现多目标自适应储蓄与获得专业财务诊断。
...
- **CFP 目标导向财富规划（GBWM）**：
  - 支持「🛡️ 应急防线」、「⚡ 提前还贷」、「🎯 阶段心愿」、「📈 资产积累」四维目标体系。
  - **智能虚拟分账（Virtual Earmarking）**：明确区分存量已分账金额与未锁定自由现金，防止资金重复计算（No Double-Counting）。
  - **优先级动态灌溉**：月度净自由结余资金自适应灌溉至各目标池，精确推算各目标的预计达成年月（ETA）与顺延风险。

- **动态现金流沙盘与双轨推演（Dynamic Cashflow Sandbox）**：
  - **双轨对比**：基准走势（Baseline）与情景推演（Scenario）实时联动渲染。
  - **双重视图**：支持「现金余额走势折线图」与「月度三层收支/结余瀑布堆叠图」无缝切换。
  - **交互式压力测试**：内置 **收入波动冲击滑块 ($-50\%\sim +50\%$)**、**单笔突发大额开支模拟** 与 **还贷策略仿真**，支持一键重置或固化为新基准。

- **负债加速策略引擎（Debt Paydown Strategy）**：
  - **雪崩法 (Debt Avalanche)**：算法优先偿还利率最高的贷款，计算最大节省利息总额。
  - **滚雪球法 (Debt Snowball)**：算法优先结清本金最小的贷款，最快注销负债笔数。
  - **机会成本权衡**：直观展示提前还贷对无债结清日的提前效应以及对其他心愿目标的延期影响。

- **CFA/CFP 三维健康诊断与智能行动建议**：
  - **流动性韧性**：应急储备覆盖月数（动态基于生活基准+月供计算）、现金筑底水位。
  - **偿债承载力**：计算 **偿债收入比（DSR）**（$<35\%$ 稳健 / $35\%\sim 45\%$ 警戒 / $>45\%$ 高危）、加权平均负债成本（WACD）。
  - **智能建议引擎（CFP Actionable Insights）**：根据整体财务健康度自动生成明确、可执行的专业理财指引。

- **4-Tab 专业规划工作台**：
  - **概览 (Overview)**：高层决策中枢、CFP 智能体检卡、近期 30 天还款与偿债缓冲。
...
├── Views/
│   ├── Overview/ # 概览中枢与 CFP 智能体检卡 (CFPInsightsCard)
│   ├── Planning/ # 规划沙盘 (DynamicCashFlowSandboxView) 与目标矩阵 (GoalListView)
│   ├── Assets/   # 资产与分账拆解
│   ├── Liabilities/ # 负债与还款日程
│   └── Settings/ # 设置与双画像切换
├── Components/   # 通用 SwiftUI 组件与三段式进度条
